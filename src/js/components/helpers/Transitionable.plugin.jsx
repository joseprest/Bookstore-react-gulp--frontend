...
const Transitionable = React.createClass({
    ...
    render() {
        ...
        const className = ['transitionable-views'];
    ...
    renderChildren(child, index) {
        ...
        const className = ['transitionable-view'];
    ...
    componentWillReceiveProps(nextProps) {
        ...
        _.each(this.state.children, (child) => {
            const existingChild = _.find(nextChildren, c => c.key === child.key);
        ...
        if (this.props.children !== nextProps.children) {
            ...
            _.each(nextChildren, (child) => {
                const existingIndex = _.findIndex(allChildren, c => c.key === child.key);
                if (existingIndex === -1) {
    ...
    componentDidUpdate(prevProps, prevState) {
        const currentChildrenKey = _.map(this.getChildrenAsArray(this.props.children), 'key').join(
            '|',
        );
        const prevChildrenKey = _.map(this.getChildrenAsArray(prevProps.children), 'key').join('|');
    ...
    transitionChildren(opts) {
        ...
        const allCurrentKeys = _.map(currentChildren, 'key');
        const currentKeys = _.map(this.state.children, 'key');
        const nextKeys = _.map(newChildren, 'key');
    ...
    callTransitions(transitions, opts) {
        const keys = _.map(transitions, 'key');
        const inKeys = _.map(_.filter(transitions, ['direction', 'in']), 'key');
        const outKeys = _.map(_.filter(transitions, ['direction', 'out']), 'key');
        const otherKeys = _.map(_.filter(transitions, ['direction', 'other']), 'key');
        ...
        const remainingTransitionsKeys = _.difference(
            _.map(remainingTransitions, 'key'),
            transitioningOther,
        );
    ...
    callTransition(direction, key, opts, done) {
        ...
        const child = _.find(this.state.allChildren, c => c.key === key);
        ...
        const onTransitionDone = _.bind(function () {
